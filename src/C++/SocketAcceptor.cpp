...
void SocketAcceptor::onConnect( SocketServer& server, SOCKET a, SOCKET s )
{
...
void SocketAcceptor::onWrite( SocketServer& server, SOCKET s )
{
  SocketConnections::iterator i = m_connections.find( s );
  if ( i == m_connections.end() ) return ;
...
bool SocketAcceptor::onData( SocketServer& server, SOCKET s )
{
  SocketConnections::iterator i = m_connections.find( s );
...
void SocketAcceptor::onDisconnect( SocketServer&, SOCKET s )
{
  SocketConnections::iterator i = m_connections.find( s );
  if ( i == m_connections.end() ) return ;
  SocketConnection* pSocketConnection = i->second;
